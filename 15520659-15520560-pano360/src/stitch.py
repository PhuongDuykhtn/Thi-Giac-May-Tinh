...
def RANSAC(matched_pairs, prev_shift):
    ...
    max_inliner = 0
    for k in range(K):
        #chọn ngẫu nhiên 1 cặp điểm đặc trưng
        ...
        #ngưỡng (threshold_distance) sai số để xác định điểm nào đó có khớp mô hình không
        inliner = 0
...
Input:
    img1: hình 1
    img2: hình 2
    shift: quan hệ vị trí giữa hai hình
    pool: chủ yếu dùng để sử dụng tối đa CPU
    blending: khi hai màu sắc hai hình không hài hòa thì chỉnh màu sắc
...
def stitching(img1, img2, shift, pool, blending=True):
    ...
    cv2.imwrite('aligned.jpg', shifted_img1)

    # Cắt những vùng không cần thiết
...
"""
Được dùng để căn chỉnh cho hình cân đối, không bị lệch

Input:
    img: Ảnh panorama
    shifts: dùng để chỉnh cho ảnh pano được cân đối
...
"""
Loại bỏ những vùng đen của ảnh
