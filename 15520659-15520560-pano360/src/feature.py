...
"""
Trích chọn bộ descritpor từ corner response image

Các tham số:
    corner_response: corner response matrix
    kernel: kích thước cửa sổ của descriptor, có số chiều là kernel^2 
...
"""
Tìm trên 2 bộ descriptors những cặp điểm chung


Các tham số:
    descriptor1:
    descriptor2:
    feature_position1: vị trí điểm đặc trưng ảnh 1
    feature_position2: vị trí điểm đặc trưng ảnh 2
    pool: for mulitiprocessing
    y_range: Tìm cặp điểm chung bằng cách so sánh tung độ của nó

Kết quả trả về:
    Các cặp điểm chung, trả về ma trận Nx2x2
"""
